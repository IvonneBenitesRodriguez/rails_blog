...
RSpec.describe 'User Features', type: :feature do
  before do
    @author_user = User.create!(name: 'Ella', photo:
    'https://example.com/ella_profile.png',
                                bio: 'Educator from Peru.')
    @commenter_user = User.create!(name: 'Ivonne Benites',
                                   photo: 'https//example.com/may_profile.png',
                                   bio: 'Junior Student from Peru.')

    @post = Post.create!(title: 'Wayy', text: 'Hello, this is my first post.', comments_counter: 0,
  ...
  describe 'User Profile Page' do
    ...
    it "displays post's title" do
      visit user_posts_path(@commenter_user)
      expect(@post.title).to have_content('Wayy')
    end
    ...
    it 'displays comments' do
      ...
      expect(@post.comments.map(&:text)).to include('My wonderful comment.')
    end
...
>>>>>>> e10d7a1cfe72f16bc59947faf3f01194d551d711
end
