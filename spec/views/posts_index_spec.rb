...
RSpec.describe 'User Features', type: :feature do
    before do
        ...
        4/41/Profile-237.png'),
        bio: 'Mastology doctor')
        @patient = User.create!(id: 6, name: 'Patient', 
        photo:
        'https://upload.wikimedia.org/wikipedia/commons/4/41/Profile-237.png',
        ...
        Post.create!(title: 'Bienvenidos!!', text: 'Hola a todos, bienvenidos.',
        comments_counter: 0, 
        ...
        @patient_post = Post.create!(title: 'First appointment at the hospital',
        text: 'My first visit to the hospital!',
    ...
    describe 'User Post Page' do
        it 'displays post details' do
            visit user_posts_path(@patient)
            expect(@patient.posts[0].title).to
            have_content('First day at the hospital')
            expect(@patient.posts[0].text).to have_content
            ('Great moments on my first day!')
        end
        ...
        view' do
            5.times do |n|
                Port.create!(title: "Test Post #{n + 1}", text: "Test post text
                #{n + 1}",
                comments_counter: 0, likes_counter: 2, author: @patient)
            end
